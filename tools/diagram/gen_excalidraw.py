...
y6 = ty + 230
txt("6. The functions we need — and what does them today", X, y6, 24)
txt("named for what they must do, not for the skill that happens to do it", X, y6 + 34, 15)
...
for layer, fns in FUNCTIONS:
    h = 40 + len(fns) * 56
    lc = RED if all(f[3] in ("GAP", "unused") for f in fns) else INK
    rect(X, fy, W, h, stroke=lc)
    txt(layer, X + 18, fy + 10, 18, lc)
    ry_ = fy + 40
    for name, must, today, status in fns:
        colour, fill = STATUS[status]
        box(name, X + 18, ry_, 250, 46, stroke=colour, bg=fill, size=14)
        txt(must, X + 285, ry_ + 13, 13)
        label = today if today else "— nothing —"
        txt(label, X + 880, ry_ + 6, 13, colour)
        txt(status.upper(), X + 880, ry_ + 26, 12, colour)
        ry_ += 56
    fy += h + 14

txt("5 gaps  ·  3 built but unused  ·  10 working.\n"
    "\"Route to the altitude\" is the keystone: nothing performs it, and it is what would\n"
    "decide whether the three unused ones are ever reached.", X + 18, fy + 10, 15, RED)
