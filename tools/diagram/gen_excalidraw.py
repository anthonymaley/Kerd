...
y6 = ty + 230
txt("6. The functions we need — in, grounding, out, acceptance", X, y6, 24)
txt("in execution order. GROUNDING is what must be read before anything is produced — "
    "inputs arrive on their own, grounding is what gets skipped.\n"
    "a function shows detail only once it has been interviewed. blank means not yet "
    "walked, not nothing to say.", X, y6 + 34, 15)
...
def _rowh(*fields):
    """Row height follows the tallest column, not a fixed guess."""
    lines = max((f.count("\n") + 1) for f in fields if f) if any(fields) else 1
    return max(ROW_H, int(lines * 12 * 1.25) + 22)

# header row
txt("FUNCTION", C_FN, y6 + 92, 13)
txt("TODAY", C_TD, y6 + 92, 13)
txt("IN", C_IN, y6 + 92, 13)
txt("GROUNDING  — read before producing", C_GR, y6 + 92, 13)
txt("OUT", C_OUT, y6 + 92, 13)
txt("ACCEPTANCE  — two keys", C_ACC, y6 + 92, 13)

NOT_WALKED = "— not yet interviewed —"
...
for layer, fns in FUNCTIONS:
    ...
    heights = [_rowh(r[3], r[4], r[5], r[6]) for r in rows]
    h = 38 + sum(heights)
    lc = RED if all(f[2] in ("GAP", "unused") for f in fns) else INK
    rect(X, fy, W6W, h, stroke=lc)
    ...
    for (name, today, status, ins, gr, outs, acc), rh in zip(rows, heights):
        ...
        txt(gr if gr else NOT_WALKED, C_GR, ry_ + 8, 12,
            INK if walked else RED)
        txt(outs, C_OUT, ry_ + 8, 12)
        txt(acc if acc else NOT_WALKED, C_ACC, ry_ + 8, 12,
            INK if walked else RED)
        ry_ += rh
    fy += h + 14
...
qy = y9 + 74
# Row height follows the MUST text, not the box. Requirements grow as they are
# detailed out one by one; fixed spacing silently stacked them on top of each
# other the first time one ran past two lines.
def _req_row_h(must):
    return max(62, int((must.count("\n") + 1) * 13 * 1.25) + 20)

for layer, reqs in REQUIREMENTS:
    rows = [_req_row_h(must) for _, must in reqs]
    h = 38 + sum(rows)
    rect(X, qy, W6, h, stroke=INK)
    txt(layer, X + 18, qy + 9, 17)
    ry_ = qy + 36
    for (name, must), rh in zip(reqs, rows):
...
# ══ 11. Where we are — the diagnostic, moved out of the requirement row ═══
# This column used to sit inside movement 6 under "how we would know", where it
# read as part of the requirement. It is not: it says nothing about any output,
# only whether the function is running at all. Its own view — and the first
# concrete instance of the "Show where we are" function.

y11 = ry_ + 100
txt("11. Where we are — what exists, and what is still a gap", X, y11, 24)
txt("not a requirement. a reading of the system as it stands today: does this "
    "function run at all, and what would you point at to say so.", X, y11 + 34, 15)

txt("FUNCTION", X + 18, y11 + 74, 13)
txt("STATE", X + 268, y11 + 74, 13)
txt("WALKED?", X + 430, y11 + 74, 13)
txt("ACHIEVED WOULD LOOK LIKE  /  THE GAP TODAY", X + 620, y11 + 74, 13)

_all_fns = [f for _, fns in FUNCTIONS for f in fns] + list(CROSSCUTTING)
_dh = [_rowh(f[5]) for f in _all_fns]
sy11 = y11 + 100
rect(X, sy11, W6, 24 + sum(_dh), stroke=INK)
ry_ = sy11 + 16
for (name, today, status, ins, outs, mea), rh in zip(_all_fns, _dh):
    colour, fill = STATUS[status]
    box(name, X + 18, ry_, 232, 46, stroke=colour, bg=fill, size=13)
    txt(status.upper(), X + 268, ry_ + 14, 12, colour)
    walked = name in DETAIL
    txt("interviewed" if walked else "not yet", X + 430, ry_ + 14, 12,
        INK if walked else RED)
    txt(mea, X + 620, ry_ + 4, 12, colour)
    ry_ += rh

txt(f"{len(DETAIL)} of {len(_all_fns)} functions interviewed. "
