...
def chip(ref, cls=""):
    return ('<a class="chip %s" href="#%s" data-goto="%s">%s</a>'
            % (cls, ref, ref, ref))
...
def render(preamble, recs, graves, notes, register_hash, source_path):
    ...
    for t in targets:
        ...
        parts.append(
            ...
            % ("" if alive else " missing", html.escape(t), html.escape(title),
    ...
    for r in recs:
        ...
        traces_html = "".join(
            '<span class="tag%s">%s</span>' % (
                " law" if t.lower().startswith("law") else "", html.escape(t))
            for t in r["traces_list"]) if not r["untraced"] else \
...
.card.hit{outline:3px solid var(--derive);outline-offset:2px}
...
function applyEdits(card, ref){
  ...
  $$(".field,.linkcol",card).forEach(el=>{
    ...
    if(name==="depends on"){
      ...
      v.innerHTML = refs.length
        ? refs.map(r=>'<a class="chip" href="#'+r+'" data-goto="'+r+'">'+r+'</a>').join("")
        : '<span class="muted">none</span>';
    } else if(name==="traces to"){
      v.innerHTML = val.split(",").map(t=>t.trim()).filter(Boolean)
        .map(t=>'<span class="tag'+(/^law/i.test(t)?" law":"")+'">'+esc(t)+"</span>").join("");
...
document.addEventListener("click",e=>{
  ...
  if(g){ e.preventDefault(); const t=document.getElementById(g.dataset.goto);
