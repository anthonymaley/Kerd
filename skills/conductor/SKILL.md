...
- **Optional effort hint per delegated step:** a `[delegate]` step may append a reasoning-effort level — `[delegate, effort: low]` — passed to the subagent at dispatch. Effort and model tier are two independent levers: mechanical edits → `effort: low` on a small model; standard implementation → `effort: medium`; core-but-delegatable work → `effort: high` on a bigger model. Spec-authoring itself is the high-effort work the planning model spends its own tokens on. Omit the hint and the subagent runs at its default effort.
