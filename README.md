...
**The progress render can no longer go stale — and the ship flow changed.** CI's seventh step re-renders the board in a temp dir at every pushed tip and byte-compares it against the committed pair; different or missing turns the push red with the fix quoted verbatim. Every push now goes: work commit → refresh the render → render commit → one push. Forgetting is safe — the machine refuses and tells you exactly what to run. (It has caught five real slips since, including one authored by the orchestrator's own reasoning.)

*Release notes for v0.77.0 and earlier live in git history — `git log --follow README.md`.*
