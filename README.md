...
withdraws one until the restart begins. It is for Claude only, and it carries the model and
effort; a session started with other launch options, or with Claude settings in its environment
that tmux would not carry, is not rolled, since a restart would drop them. Outside
