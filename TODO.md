...
**release-closeout at goal, one move from done — the seventh gate-routed item, v0.85.0** (frame 07ade2c · design GO 392d8b1 · build 2b80dc6 · amendment a8ddc4d · maiden pass 0bdd38f). The expert-user pass = the 2026-08-06 ~13:20 close (the pass fired at the close that shipped it; recorded in that session log). Next session: goal record `docs/gates/` citing the observation → loop rung. Earlier the same day: conductor-boundary closed (sixth walk, 133ca42), vault-unhook closed (fifth walk, db9b47c), v0.83.1 fence-aware parsers.

Next pick after the record: Backlog High freshest — time-awareness · auto-sizing · stop-hook · release-closeout slice 2 (declared external surfaces).
...
**High consequence**
- **time-awareness** (Tony 2026-08-06, 12:30 EDT — named while the model was writing "late-evening sitting" at midday; the item then dated itself "midnight" before checking `date`, proving the point twice): the model can't tell time and misjudges elapsed effort. Two halves: (1) **statusline** (Tony's follow-up) — a `date`-driven segment renders the time to the human on every update; trivial, but the model doesn't see the statusline; (2) **capture at structural moments** — `date` from the command line (free, local) at conductor dispatch/verify/close, switch boundaries, spec step completions, recorded into the artifacts (session logs, spec steps, gate records) so duration-per-task becomes derived data the model reads. Feeds the parked journey view's surfaced requirement (progress % and time-left need declared on-disk homes). Frame question: which moments earn a timestamp, and where does effort-per-rung live on disk.
...
- **state-contract warts for the pass's first self-test** (cold eyes 2026-08-06 v0.85.0 observations): the interaction matrix marks slainte `-` for TODO.md while slainte's docs-check 6 reads it and the new charter files Backlog rows; and the "Others must NOT" column now holds a slainte description, not a prohibition. Both are exactly what `/kerd:slainte release` rule 4 should flag — leave for the pass or fix by hand, either way one commit.
