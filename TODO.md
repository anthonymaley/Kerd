...
## Current Session
(2026-06-28 — big design session: shipped focus + jit + capturerequirements + spine-wiring (v0.42.0→v0.45.0); reopened and redesigned the whole mode system into the "dial" idea→launch lifecycle; established a partner-mode working agreement. Tree clean, pushed.)

### What's next
- **Build the dial — the mode/lifecycle redesign (major initiative).** Full design + implementation plan at `docs/plans/2026-06-28-mode-lifecycle-redesign.md`. Architecture (3 altitudes): **switch** (boundary) / **coach** (= renamed `dian`, session conductor) / **dial** (lifecycle PM that walks an idea through explore→validate→plan→build→launch, rigor rising, JIT decision-style throughout, start/jump/rewind). All 5 stages are spec'd. Start with **Phase 0 decisions** (gate the build): renames (`dian`→coach + final `dial` name); dial = one skill vs 5 modes (lean one); skills-split (lean stay-one-plugin); prune the tool-use "modes" (legal/sales/writing/research/strategy/maintain). Then Phase 1 (dial conductor), Phase 2 (train each stage as a slice, eyeball-gated).
