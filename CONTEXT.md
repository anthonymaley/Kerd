...
1. **Context/history split — BUILT (2026-07-03), refined 0.61.0 (2026-07-06).** Test 1 (A/B quiz) and Test 2 (manual lean cycle) both passed; the split shipped as v0.60.0: state (CONTEXT.md, overwritten) / work (TODO.md, `## Now` + `## Backlog`) / history (kivna/sessions/, immutable). Switch-in reads exactly those three files. Switch-out gained closure inference (done/open/unsure verdicts on open TODOs, informational list, unsure tagged `(done? — confirm)` for one switch-in question) and a prompt-free vault save. First switch-out on other repos self-migrates them. This file is now owned by switch. **v0.61.0** added: switch-in Summarize ends with a full numbered TODO pick-list (`## Now` + `## Backlog`, no truncation), and focus's question rule became a quality bar (multiple-choice allowed only when it clarifies a real choice — 2-4 crisp options — never a lazy binary or vague menu), enforced in `hooks/focus.sh`. **v0.62.0** (2026-07-07) added conductor **delegated execution**, toggled on with `/kerd:conductor fable on` (default off, session-scoped): the plan becomes a committed spec file (`docs/plans/YYYY-MM-DD-<slug>-spec.md`) with `[fable]`/`[delegate]` step tags; execute dispatches `[delegate]` steps to cheaper models via in-session subagents (default — Fable reviews their evidence) or a fresh-session handoff. Written-but-not-run-live.
...
- **Conductor delegated execution: Fable plans, cheaper models build** (2026-07-07): toggled on with `/kerd:conductor fable on` (default off, session-scoped on the conductor line in `.active-modes`) — an explicit assertion you're on a top-tier model, **not** model auto-detection (rejected an earlier auto-detect gating; conductor can't reliably know its own model, and Fable credits vary session to session). When on, the plan becomes a committed spec file (`docs/plans/YYYY-MM-DD-<slug>-spec.md`), each step tagged `[fable]` (done inline) or `[delegate]` (handed off). Dispatch is either in-session subagents (default — Fable spawns Sonnet/Haiku and reviews their evidence, staying in judgment and out of the grind) or handoff to a fresh cheap session. **The spec is the contract** — a vague `[delegate]` step yields a confidently-wrong build with no recourse, so spec quality is the safety mechanism and the reason the expensive planning tokens are worth spending. Off (the default, and whenever the toggle isn't flipped) conductor plans and builds inline on the active session model exactly as before.
- **Multiple-choice questions are a quality bar, not a ban** (2026-07-06): offer options only when they *clarify* a real choice that's the user's to make (2-4 crisp, distinct options) — never a lazy binary that offloads a call Claude should make, never vague or verbose. Lives in the focus hook now; a ready-to-drop phrase for promoting it to global `~/.claude/CLAUDE.md` sits in the backlog.
...
- Surface model-tiered delegation in the top-level plugin capability list (plugin.json / marketplace.json `description`), or leave "session discipline" to subsume it? Left unchanged in v0.62.0 pending a call.
