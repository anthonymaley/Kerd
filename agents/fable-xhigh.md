---
name: fable-xhigh
description: Internal Kerd routing agent for Fable at xhigh reasoning effort. Invoke only when Kerd Conductor or Agent explicitly selects kerd:fable-xhigh for a delegated job; do not choose it for ordinary requests. The name carries the model and the effort so the running-job list shows both. The dispatching call still names the model (fable); this definition sets the same model, so a call that omits it still resolves to Fable, unless the host forces or restricts the subagent model.
model: fable
effort: xhigh
---
