...
**Historical: the modes system was cut in v0.75.0, so no `conductor:` line is written
any more and this fallback is gone.** Switch Out fell back to the `conductor:` line in `kivna/.active-modes` when no
...
- **A `## Risk ledger` section holds the table and NOTHING else — prose after it becomes phantom malformed rows** (2026-09-02, found while framing `question-set-staleness`). `parse_ledger` treats every non-empty line in the section body as a row, so an explanatory paragraph below the table reported as `row 5: expected 8 columns, found 1` through `row 12`, inflating the scope rung from `need 9` to `need 17`. **The refusal is loud but points at the wrong thing** — it names row numbers that do not exist as rows, so the natural reading is that the table is malformed rather than that prose is being parsed. Put the rationale in `## Value` or a section of its own. Sibling of the fence-aware fix of v0.83.1 (`2a0ea4a`): there, quoted structure inside a fenced block was being read as real; here, unfenced prose inside a structured section is. **Nothing distinguishes a ledger's prose from its rows, and that is the gap** — filed in TODO rather than fixed, since a parser change belongs to its own rung.
...
**Historical, as above: nothing writes a conductor line since v0.75.0.**
`kivna/.active-modes` held one conductor line. A second task in the same
...
- **The `.active-modes` line contract spanned four files** (historical: only Skriv's line remains, and Switch no longer reads this file; the conductor line went with the modes system in v0.75.0). A skill's `.active-modes` line was written by the skill, grepped by the session hooks (`hooks/session-start.sh` and `hooks/skill-complete.sh` read `^mode:`), asserted by `tests/hooks_test.sh`, and was read by Switch. Renaming the line prefix (`dian:` → `conductor:`) must change all in one commit or the hook/test harness breaks quietly. The harness is the safety net — run it immediately after to confirm the contract held. (Before v0.96.0 `hooks/stop.sh` also grepped `^conductor:`; it was cut when hooks moved to plugin auto-load.)
...
- **A daemon never loads a TUI, so `thread/read` reports every TUI as `notLoaded`** (2026-09-11, Fable's final review of 0.109.0): "not loaded" is the normal state of a session a person opened, not "offline". A refusal keyed on it refuses every TUI. Route on what the store row *is* (`source='cli'`, `thread_source='user'`), not on the daemon's status.
- **`codex queue` declares its own `-C/--cd`** (2026-09-11): a `head -20` of `codex queue --help` cut it off and the adapter placed the flag before the subcommand. Read help in full before asserting a flag is global-only; pin flag/value pairs in tests, not argv positions.
- **UUIDv7 prefixes collide by timestamp** (2026-09-11): Codex threads spawned in the same second share eight hex characters, so a truncated listing shows "duplicates" that are distinct threads. Count distinct full ids before calling a dedupe bug.
- **A prose mention of both request markers parses as a reply** (2026-09-11, Fable): "I'll put it between `<b>` and `</b>` once…" yielded `reply='and'` and the real answer was never retrieved. Retrieval must require the markers to delimit lines, judged *before* trimming to the last marker, and events must join by native message id so a split reply keeps its text.
- **`git add` aborts the whole command on one unmatched pathspec** (2026-09-11): listing a pre-move path beside new ones staged nothing, leaving a `git mv` rename at `R100` — an unmodified rename with none of the work in it. Read `git diff --cached --stat`, not the command's silence, before committing.

- **A transient `.git/index.lock` from another session fails a commit; the staged set survives** (2026-09-11, twice): check for a live git process, confirm the lock is gone, retry. Never delete it blind.
- **A review subagent finds nothing in your own build by reading; it finds things by testing the hazards a prior reviewer named** (2026-09-11): the one real bug a self-review caught came from a guard test written against Codex's correction (a), not from re-reading the code. And a claim about test coverage wants the same proof as a claim about code — one mutation the record said was caught was not, until the test existed.
