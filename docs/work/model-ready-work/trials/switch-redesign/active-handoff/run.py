...
def check_preserved(root, commit, files):
    if handoff.git(root, "rev-parse", "HEAD") != commit:
        raise RuntimeError("Worker changed Git history")
    if any((root / name).read_bytes() != content for name, content in files.items()):
        raise RuntimeError("Worker changed protected inputs or saved place")
...
def main():
    ...
    handoff.git(source, "add", "--", "agreement.md", "example.json", ".gitignore")
    ...
    class SaveBeforeShutdown(original_server):
        ...
        def receive(self, *a, **kw):
            ...
            if method == "turn/started":
                ...
                check_preserved(source, initial_commit, {"agreement.md": agreement, "example.json": example})
    ...
    place_bytes = (dest / "handoff.json").read_bytes()
    before = json.loads(place_bytes)
    ...
    for candidate in dst.get("checkpoint_candidates") or [dst["reply"]]:
        after = roll.check_state(roll.parse_reply(candidate), dest, after)
    ...
    check_preserved(dest, saved["commit"], {"agreement.md": agreement, "example.json": example,
                                           "handoff.json": place_bytes})
