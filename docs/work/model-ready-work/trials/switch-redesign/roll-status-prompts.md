# Actual worker prompts — first build and correction

These are the submitted outcome-first prompts, not reconstructed examples.
Native request/session IDs and private transport records are excluded. Build model:
gpt-5.6-sol, high effort. The failed first transport and controller-assisted recovery
are documented in [the proof](conductor-roll-proof.md). Order below follows
saved request enumeration, not an asserted execution sequence; the saved-place
content identifies initial build versus correction.
...
{
  "status": "continue",
  "next_action": "Make the two supported compatibility corrections below, add regression tests, run the agreed suite, then return for independent reassessment. Use observed Roll requests if issued; do not invent extra pieces.",
  "memory": "Conductor inspected the first interrupted transport run: all three known source processes are gone; code and docs remain. Controller independently ran all 11 tests successfully. Claude reviewed all deliverables. Runtime history statuses are continue/review/blocked, but graceful unknown historical strings are a useful bounded M3 correction: show unknown for unfamiliar strings while preserving a valid current panel; wrong JSON types still error and unknown current status remains nonzero. Second definite integration correction: normal non-observed Roll history can contain context_readings:null; accept that unavailable optional field rather than refuse the panel. Add negative/positive regression coverage and update USAGE only if needed. Do not add unrelated hardening. Retain existing private-field suppression. Existing M1=2/M4=1 are first worker's conservative reported failure counts and must not reset; future counts concern failed correction attempts, not number of failing assertions. No independent reviewer or background job is still running. After these changes the controller arranges a different-model reassessment. No new user decision is needed.",
  "evidence": [
    "roll_status.py",
    "test_roll_status.py",
    "USAGE.md"
...
# Saved place
{
  ...
  "memory": "Direction and build authority are settled. This is Conductor's isolated Studio integration task. Nothing has been built yet. No user question is pending. Do not touch live Kerd or Seinn. Finish at review, not self-acceptance.",
  "evidence": [],
  "failures": {
    "M1": 0,
    "M2": 0,
