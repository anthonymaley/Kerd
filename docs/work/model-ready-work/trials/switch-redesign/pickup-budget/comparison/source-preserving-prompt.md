Restore the local Seinn snapshot using the complete candidate instructions and
source records below. This is the same isolated local-only memory experiment,
not the historical device handoff. No project edits, network, fetch/push, tests,
builds, devices/SSH, messages, new work, delegation, native histories or app tools.
The supplied packet is verified local identity, with complete source selections.
Stop after restoration, without asking a user question or claiming a budget pass.

Return a short screen summary of position, last result and next action, then a
separate portable working-state annex. Preserve open decisions, operative
constraints, evidence caveats and deeper-reading routes from the loaded sources,
with their actual conditions in source wording where paraphrase would weaken
them. Do not collapse these into labels or silently settle open experience
choices. Retain source labels and saved-versus-fresh attribution. The annex may
be longer than the display; brevity must not erase needed meaning. No additional
history read is needed unless these source records leave a genuine relevant gap.
This annex is retained evidence for this experiment, not a new living project
rules file. Do not simply assert the source was understood: carry its working
meaning so another reviewer can check it.
