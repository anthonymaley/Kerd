...
- **Wrong-by-hours labels in new artifacts: 2 observed → 0.** A time
  may be written only in the same turn as a `date` invocation (the
  same-turn rule).
