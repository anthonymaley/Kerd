...
- **Lost is a checkable state** — today: no mechanism can detect an
  unreachable or vanished grounding artifact; target: every rung's
  declared grounding is machine-readable data, and a reference that
  does not resolve on disk goes red in CI naming the file and the
  broken reference. **0 broken grounding references at any pushed tip.**
...
audit** (Tony's call, 2026-08-05): each rung's grounding becomes
machine-readable data in the gate table's declared landing site (the
`grounding` slot per rung, empty since A8), and a CI audit rule proves
every declared grounding reference resolves on disk — a broken
reference is a refusal naming the file and the reference. The slice's
