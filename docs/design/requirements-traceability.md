...
| The five-state lifecycle | ◎ | △+ — needs a custom grammar | △- — BANANA passed, our checker | ○ | △- — closed four, state in tags | △- — the field is ours |
...
| StrictDoc | The five-state lifecycle | declare a custom grammar field `TYPE: SingleChoice(proposed, qualified, final, superseded, dropped)` with `REQUIRED: True`, which refuses a bad value by name and an omission by name | permanent | high — built and verified hands-on against a Kerd-shaped grammar; but it IS a countermeasure, because the shipped `STATUS` field is an unconstrained free string (`STATUS: BANANA_NOT_A_REAL_STATUS` round-tripped clean, and the `Draft / Active / Deleted` rule in the grammar is dead code referenced by no production) | |
